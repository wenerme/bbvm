...
Grammar
  = exps:(
	_
    exp:(
    comment:Comment {return {comment,type:'COMMENT'}}
    / label:Label{label.type='LABEL';return label}
    / Expression
    ) _ comment:Comment? (NL/EOF){
      if(comment){exp.comment=comment}
...
Pseudo
  = op:'.BLOCK'i _ size:Value _ value:Value {return {op,size,value}}
  / op:'DATA'i _ symbol:Identifier _ dt:PSEUDO_DATA_TYPE? _ v:Value values:(_ ',' _ v:Value{return v})* {values.push(v);return {op,symbol,dt,values}}
...
Instruction
  = op:( 'EXIT'i / 'RET'i / 'NOP'i) {return {op}}
  / op:( 'CALL'i / 'PUSH'i / 'POP'i / 'JMP'i ) _ a:Operand {return {op,a}}
  / op:( 'IN'i / 'OUT'i ) _ a:Operand _ ',' _ b:Operand {return {op,a,b}}
  / op:( 'CAL'i ) _ dt:DATA_TYPE _ cal:CAL_TYPE _ a:Operand _ ',' _ b:Operand {return {op,cal,dt,a,b}}
  / op:( 'LD'i / 'CMP'i ) _ dt:DATA_TYPE _ a:Operand _ ',' _ b:Operand {return {op,dt,a,b}}
  / op:( 'JPC'i ) _ cmp:CMP_TYPE _ a:Operand {return {op,cmt,a}}

Label
  = name:Identifier _ ':' {return {symbol:name,label:true}}

Value
  = v:$([1-9][0-9]*'.'[0-9]+){return {type:'FLOAT',value: parseFloat(v)}}
  / v:$('0x'i [a-f0-9]i+ / '0' [0-7]* / [1-9][0-9]*) {return {type:'INTEGER',value: parseInt(v)}}

Operand
  = symbol:Identifier {return {direct:true,symbol}}
  / value:Value {return {direct:true,value}}
  / '[' symbol:Identifier ']' {return {direct:false,symbol}}
  / '[' value:Value ']' {return {direct:false,value}}
...
SingleLineComment
  = ( ';' / "'" ) content:$(!LineTerminator SourceCharacter)*{return content.trim()}
